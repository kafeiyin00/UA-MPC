...
ros::Publisher pub_cloud, pub_pose, pub_intercloud, pub_dyncloud, pub_uavcloud, depth_img_pub_, comp_time_pub, pub_collisioncloud, pub_rotorangle;
sensor_msgs::PointCloud2 local_map_pcl;
sensor_msgs::PointCloud2 local_depth_pcl;
ros::Subscriber odom_sub;
ros::Subscriber rotorspeedcontrol_sub;
...
// Jianping 
double rotorSpeed = 3.14/2;
double currentRotorAngle;
Eigen::Matrix4f sensor2rotorframe, rotorframe2rotor, rotor2body;
...
void rcvRotorSpeedCallbck(const std_msgs::Float32::ConstPtr& msg)
{
  // std::cout<<"set rotor speed: " << msg->data <<"\n";
  rotorSpeed = msg->data;
}
...
void renderSensedPoints(const ros::TimerEvent& event)
{
  ...
  currentRotorAngle = currentRotorAngle + rotorSpeed/sensing_rate;
  rotorframe2rotor.setIdentity();
  rotorframe2rotor.block<3,3>(0,0) = Eigen::AngleAxisf(currentRotorAngle,Eigen::Vector3f::UnitZ()).toRotationMatrix();
  // std::cout<<"currentRotorAngle : " << currentRotorAngle <<"\n";
  sensor2rotorframe.setIdentity();
  sensor2rotorframe.block<3,3>(0,0) = Eigen::AngleAxisf(3.14/6,Eigen::Vector3f::UnitX()).toRotationMatrix();
  ...
  body2world.block<3, 1>(0, 3) = Eigen::Vector3f(odom_.pose.pose.position.x, odom_.pose.pose.position.y, odom_.pose.pose.position.z);;

  Eigen::Matrix4f sensor2world = body2world * rotor2body * rotorframe2rotor* sensor2rotorframe;

  const Eigen::Matrix3f rot = sensor2world.block<3,3>(0,0);
  const Eigen::Vector3f pos = sensor2world.block<3,1>(0,3);
  ros::Time time_stamp_ = odom_.header.stamp;
    ...
    render.render_pointcloud(local_map,pos,Eigen::Quaternionf(sensor2world.block<3, 3>(0, 0)),time_frominit);
    ...
    quadrotor_msgs::Float64Stamped rotor_angle_msg;
    rotor_angle_msg.header.stamp=time_stamp_;
    rotor_angle_msg.data = currentRotorAngle;
    pub_rotorangle.publish(rotor_angle_msg);
...
int main(int argc, char** argv)
{
  ...
  // calibration
  std::string calibration_angle_string;
  nh.param("R_B_R",calibration_angle_string,std::string("0;0;0"));

  float r_z,r_y,r_x;
  sscanf(calibration_angle_string.c_str(),"%f;%f;%f",&r_z,&r_y,&r_x);
  rotor2body.setIdentity();
  rotor2body.block<3,3>(0,0) = (AngleAxisf(r_z/180.0*M_PI, Vector3f::UnitZ())* AngleAxisf(r_y/180.0*M_PI,Vector3f::UnitY())* AngleAxisf(r_x/180.0*M_PI, Vector3f::UnitX())).toRotationMatrix();
  ...
  render.setParameters(image_width,image_height,250,250,downsample_res,polar_resolution,yaw_fov,vertical_fov,1.5,sensing_horizon,sensing_rate,use_avia_pattern,use_os128_pattern,use_minicf_pattern);
  ...
  rotorspeedcontrol_sub = nh.subscribe("rotorspeedcontrol", 50, rcvRotorSpeedCallbck);
  
  pub_rotorangle = nh.advertise<quadrotor_msgs::Float64Stamped>("rotor_angle", 10);

  pub_dyncloud = nh.advertise<sensor_msgs::PointCloud2>("dyn_cloud", 10000);
  pub_intercloud = nh.advertise<sensor_msgs::PointCloud2>("sensor_cloud", 10000);  
  pub_cloud = nh.advertise<sensor_msgs::PointCloud2>("cloud", 10000);
  pub_pose = nh.advertise<geometry_msgs::PoseStamped>("sensor_pose", 10000);
  pub_uavcloud = nh.advertise<sensor_msgs::PointCloud2>("uav_cloud", 10000);
  depth_img_pub_ = nh.advertise<sensor_msgs::Image>("depth_img", 10000);
  comp_time_pub = nh.advertise<geometry_msgs::PoseStamped>("simulator_compute_time", 10000);
  pub_collisioncloud = nh.advertise<sensor_msgs::PointCloud2>("collision_cloud", 10000);
